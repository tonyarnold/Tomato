...
<content>
	...
	<div class="box graphs">
		...
		<div class="content">
			<div class="fluid-grid">
				...
				<table id="firstTable">
					<thead>
						<tr><th>&nbsp;</th><th style="text-align:left;">Class</th><th class="count">Connections</th><th class="pct">Percent</th></tr>
					</thead>

					<tfoot>
						<tr><td class="total" colspan="2">Total</td><td id="ccnt-total" class="total count"></td><td class="total pct">100%</td></tr>
					</tfoot>
				</table>

				<script type="text/javascript">
					for (i = 0; i <= 10; ++i) {
						$('#firstTable thead').append('<tr style="cursor:pointer" onclick="mClick(' + i + ')">' +
		...
		<div class="box graphs">
			...
			<div class="content">
				...
				<table id="secondTable">
					...
					<tfoot>
						<tr><td class="total" colspan="2">Total</td><td id="bocnt-total" class="total count"></td><td id="bocntx-total" class="total count"></td><td id="rateout" class="total pct"></td></tr>
					</tfoot>
				</table>

				<script type='text/javascript'>
					for (i = 0; i <= 10; ++i) {
						$('#secondTable thead').append('<tr style="cursor:pointer" onclick="mClick(' + i + ')">' +
							'<td class="color" style="background:#' + colors[i] + '" onclick="mClick(' + i + ')">&nbsp;</td>' +
		...
		<div class="box graphs last">
			...
			<div class="content">
				...
				<table id="thirdTable">
					...
					<tfoot>
						<tr><td class="total" colspan="2">Total</td><td id="bicnt-total" class="total count"></td><td id="bicntx-total" class="total count"></td><td id="ratein" class="total pct"></td></tr>
					</tfoot>
				</table>

				<script type='text/javascript'>
					for (i = 0; i <= 10; ++i) {
						$('#thirdTable thead').append('<tr style="cursor:pointer" onclick="mClick(' + i + ')">' +
							'<td class="color" style="background:#' + colors[i] + '" onclick="mClick(' + i + ')">&nbsp;</td>' +
							'<td class="title" style="width:45px">' + abc[i] + '</td>' +
	...
	<script type="text/javascript">$('.fluid-grid.inandout').after(genStdRefresh(1,2,"ref.toggle()"));</script>
